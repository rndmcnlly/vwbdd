...
├── slab           Slab/Diff roundtrips, extend_slab, minor-GC correctness
├── minor_gc_*     benchmark for what the clean-bytes invariant costs
└── cache_config   ManagerConfig builder tests
```

Total: 65 tests passing, 3 `#[ignore]`d sweeps. No runtime dependencies. See §8 of VWBDD.md for the agility-driven cut that retired the `NodeCodec`/`ArenaOffset` traits and the `.vwbdd` file format.
